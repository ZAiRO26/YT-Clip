...
async def get_llm_settings(db: AsyncSession = Depends(get_async_session)):
    ...
    try:
        ...
        result = await db.execute(text("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'settings');"))
...
async def update_llm_settings(settings_data: LLMSettingsSchema, db: AsyncSession = Depends(get_async_session)):
    ...
    try:
        ...
        # Upsert settings
        await db.execute(
            text("INSERT INTO settings (key, value) VALUES ('llm_base_url', :val) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value"),
            {"val": f'"{settings_data.llm_base_url}"'}
        )
        await db.execute(
            text("INSERT INTO settings (key, value) VALUES ('llm_api_key', :val) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value"),
            {"val": f'"{settings_data.llm_api_key}"'}
        )
        await db.execute(
            text("INSERT INTO settings (key, value) VALUES ('llm_model', :val) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value"),
            {"val": f'"{settings_data.llm_model}"'}
        )
        
        if settings_data.export_path is not None:
            await db.execute(
                text("INSERT INTO settings (key, value) VALUES ('export_path', :val) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value"),
